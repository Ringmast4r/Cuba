jQuery(document).ready(function() {
	jQuery(document).find('.gkNspPM-AuthorNewsSlider').each(function(i, module) {
		module = jQuery(module);

		if(!module.hasClass('active')) {
			module.addClass('active');
			new AuthorSlider(module);
		}
	});
});

var AuthorSlider = function (wrapper) {
    // Set object fields
    ...
    this.listWrapper = this.wrapper.find('.gkListWrapper');
    this.currentItem = 0;
    this.blank = false;
...
AuthorSlider.prototype.initUI = function () {
    var self = this;
...
AuthorSlider.prototype.addTouchEvents = function () {
	var self = this;
...
AuthorSlider.prototype.elementToMove = function (direction) {
    if (direction === 'prev') {
...
AuthorSlider.prototype.elementForMove = function (direction) {
    if (direction === 'prev') {
        return ((this.itemsAmount * 2) + this.currentItem + 1) % (this.itemsAmount * 2);
    }
...
AuthorSlider.prototype.animate = function (direction) {
    var self = this;
    var forMove, toMove;
...
AuthorSlider.prototype.autoAnimate = function (interval) {
    var self = this;
...
AuthorSlider.prototype.resize = function () {
    ...
    var width = this.listWrapper.innerWidth();
    if (width > 768) {
        width = width / 3;
    } else if (width >= 520) {
        width = width / 2;
    }
    this.listWrapper.find('.gkItem').each(function(i, item) {
        item = jQuery(item);
        // update width
        item.css('width', width + 'px');
    });
    // update list dimension
    this.newWidth = this.items.length * width;
    this.list.css('width', this.newWidth + "px");
};
